...
import homounikumus1.com.data2.model.weather.Explanation;
import homounikumus1.com.data2.model.weather.Notify;
...
public class DatabaseUtils {
    ...
    public static boolean isShowNotification () {
        Realm realm = Realm.getDefaultInstance();
        RealmResults<Notify> results = realm.where(Notify.class).findAll();
        if (results.size()>0)
            return results.first().isShow();
        return false;
    }

    public static void setIsNotificationShow (boolean isShow) {
        Notify notify = new Notify();
        notify.setShow(isShow);
        Realm.getDefaultInstance().executeTransaction(realm -> {
            realm.delete(Notify.class);
            realm.insert(notify);
        });
    }
    ...
    public static void explanationShowed(boolean exp) {
        ...
        explanation.setExplanation(exp);
        Realm.getDefaultInstance().executeTransaction(realm -> {
            realm.delete(Explanation.class);
            realm.insert(explanation);
        });
    ...
    public static boolean isExplanationShowed() {
        Realm realm = Realm.getDefaultInstance();
        RealmResults<Explanation> results = realm.where(Explanation.class).findAll();
        if (results.size()>0)
            return results.first().isExplanation();
        return false;
    }
